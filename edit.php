...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        header('Location: ' . url_for('activity.php') . '?id=' . $id);
        exit;
    }
...
<div class="create-layout">
  <div class="page-header">
    <h1><?= $typeEmoji[$pub['type']] ?> Editar <?= $typeLabel[$pub['type']] ?></h1>
  ...
  <form method="POST" action="">
    ...
    <div style="display:flex;gap:12px;">
      <a href="<?= url_for('activity.php') ?>?id=<?= $id ?>" class="btn btn-outline" style="flex:1;">Cancelar</a>
...
pickMap.on('click', e => { pickMarker.setLatLng(e.latlng); updateCoords(e.latlng); });
