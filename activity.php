...
$pub->execute([$id]);
$pub = $pub->fetch();
if (!$pub || $pub['status'] !== 'active') { header('Location: /dashboard.php'); exit; }
...
?>
    ...
    <div class="activity-layout">
      ...
      <div>
        <div style="margin-bottom:16px;">
          <a href="/dashboard.php" class="text-muted text-sm">
            ← Volver al mapa
          </a>
        </div>
        ...
        <div class="card mb-20" style="border: 1px solid var(--primary); background: linear-gradient(135deg,rgba(14,165,233,.06),var(--card));">
          <div style="display:flex;align-items:center;gap:14px;">
            ...
            <a href="/forum.php?event=<?= $pub['id'] ?>" class="btn btn-primary btn-sm">
              Abrir foro →
            </a>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="card mb-16">
          ...
          <a href="/profile.php?id=<?= $pub['user_id'] ?>" style="text-decoration:none;color:var(--text);">
          ...
          <a href="/profile.php?id=<?= $pub['user_id'] ?>" class="btn btn-outline btn-block btn-sm" style="margin-top:14px;">
            Ver perfil completo
          </a>
        </div>
    ...
    if (deleteBtn) {
      deleteBtn.addEventListener('click', async function () {
        ...
        const res  = await fetch('/api/delete_publication.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
