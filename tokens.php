...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buy_tokens'])) {
    ...
    if (!isset($packs[$amount])) {
        ...
        try {
            ...
            header('Location: ' . url_for('tokens.php') . '?ok=' . $amount);
            exit;
        } catch (Exception $e) {
...
<div class="tokens-layout">
  ...
  <div>
    <div class="card mb-16" style="position:sticky;top:calc(var(--topbar-h) + 16px);">
      ...
      <a href="<?= url_for('subscriptions.php') ?>" class="btn btn-primary btn-block">
        💎 Upgrade de plan
      </a>
    ...
    <div class="card">
      ...
      <a href="<?= url_for('subscriptions.php') ?>" class="btn btn-outline btn-sm btn-block">
